...
def crf_layer(interaction,hidden,hidden_new,out_sz):
   
   alpha = 50  
   beta = 1
   
   bias_mat = adj_to_bias(interaction,sizes=[hidden.shape[0]], nhood=1)
   bias_mat = tf.cast(bias_mat, tf.float32)
   hidden_extend=hidden[np.newaxis]
   
   coefs = attention(hidden_extend,bias_mat,out_sz)
   coefs = coefs[0]
...
def attention(emb,bias_mat,out_sz):
